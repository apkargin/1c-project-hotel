Процедура ОбработкаПроведения(Отказ, Режим)
    ...
    Движение.Период = Дата;
    Движение.Сумма = СуммаВсегоОплачено * (100 - Константы.КомиссияБанка.Получить() / 100);	
    Движение.ТипСтатьи = Справочники.Статьи.ВыручкаОтПродажи;
    
    Если СпособОплаты = Перечисления.СпособыОплаты.БанковскийПеревод
    Тогда 
	   	Движение = Движения.ДоходыРасходы.Добавить();
	    Движение.Период = НачалоДня(Дата + 86400);
	    Движение.Сумма = СуммаВсегоОплачено * Константы.КомиссияБанка.Получить() / 100;	
	    Движение.ТипСтатьи = Справочники.Статьи.РасходыНаУслугиБанков;
	КонецЕсли;
